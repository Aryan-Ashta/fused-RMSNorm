...
def plot_custom_bandwidth(csv_file, title, output_png):
    try:
        # Load the CSV generated by Triton
        df = pd.read_csv(csv_file)

        # Triton CSV columns: N, PyTorch Eager, Torch Compile, Fused Triton
        plt.figure(figsize=(10, 6))

        # Plot each line
        plt.plot(df['N'], df['Fused Triton'], label='Fused Triton', color='green', marker='o', linewidth=2)
        plt.plot(df['N'], df['Torch Compile'], label='Torch Compile', color='orange', marker='s', linewidth=1.5)
        plt.plot(df['N'], df['PyTorch Eager'], label='PyTorch Eager', color='blue', marker='x', linewidth=1.5, linestyle='--')
...
if __name__ == '__main__':
    ...
    # 1. Let Triton run the benchmarks and dump clean CSV data (ignoring Triton's blank plots)
    benchmark.run(show_plots=False, print_data=True, save_path='.')

    # 2. Manually read those CSVs and draw beautiful, non-blank curves!
    print("\nRegenerating plots from CSV data to bypass Triton's plotting bug...")
